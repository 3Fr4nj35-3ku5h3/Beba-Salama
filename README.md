...
A capstone project for the Ngao Labs Foundations of Data Science & AI Bootcamp, delivered in partnership with DataCamp.
---
The Problem
...
---
Key Findings
5am is the deadliest hour — not midnight. Long-distance trucks, overnight matatus, and early boda bodas converge on empty roads with exhausted drivers.
A night crash is 60% more likely to be fatal than a day crash (11.0% vs 6.9%).
Sunday carries the highest fatality rate; Friday carries the highest volume — two distinct risk profiles requiring different interventions.
Matatus appear in 8.2% of all recorded crashes, and given passenger capacity, each incident risks multiple victims.
Crash volume peaked in 2015 and has declined since — evidence that interventions have measurable effect, though the problem remains acute.
16.8% of crashes are classifiable as high-severity using inputs (time, location, vehicle type) known before the journey begins — the core premise this platform is built on.
...
Source: microdata.worldbank.org/index.php/catalog/6249
...
Source: data.humdata.org/dataset/kenya-road-accidents-database
...
├── models/
│   ├── lightgbm_high_risk.txt    # Trained classifier
│   ├── results.json              # Evaluation metrics
│   ├── feature_importance.csv
│   └── shap_importance.csv
├── notebooks/
│   └── 03_model_training.ipynb   # Full investigation: leakage audit, training, SHAP
├── src/
...
LightGBM risk classification model	✅ Done — AUC-ROC 0.61, see `notebooks/03_model_training.ipynb`
Data leakage audit (matatu/motorcycle/report-count features)	✅ Done — found and fixed before final model
SHAP explainability layer	✅ Done — `src/explain_model.py`, `models/shap_importance.csv`
...
---
Contributing
See `CONTRIBUTING.md` for branch naming, commit conventions, and PR process for team members.
---
License
Code in this repository is released under the MIT License. Data usage is subject to the terms of the original sources (World Bank, Kenya Police / HDX) — see `data/README.md`.
---
Acknowledgments
